        ...
        if($sql_b_count > 0){
            if($sql_b_check['m_id'] == $_SESSION['m_id'] || $_SESSION['m_level'] == 1){
                if(!empty($_POST['t']) || !empty($_POST['d'])){
                    ...
                    if(isset($_POST['b_id'])){
                        echo "<script>alert('กรุณากรอกข้อความหรือหัวข้อ');location='board_edit.php?b_id=$e_id';</script>";
                    }else if(isset($_POST['id'])){
                        echo "<script>alert('กรุณากรอกข้อความหรือหัวข้อ');location='board_edit.php?id=$e_id';</script>";
                    }
                }
            }else{
